...
function projectSphere(
  ...
  const clampedLat = pole === "N" ? Math.max(50, Math.min(90, lat)) : Math.min(-50, Math.max(-90, lat));
  const r =
    pole === "N"
...
const statusColors: Record<string, { bg: string; border: string; glow: string; text: string; svg: string }> = {
  Active: {
    bg: "bg-emerald-400",
...
const INDIAN_BASES = [
  { name: "Maitri Station", region: "Antarctic", lat: -70.76, lon: 11.73, desc: "Schirmacher Oasis" },
  { name: "Bharati Station", region: "Antarctic", lat: -69.41, lon: 76.19, desc: "Larsemann Hills" },
  ...
  return (
    <div className="glass relative overflow-hidden rounded-3xl glow-soft border border-primary/20 bg-card/60">
      ...
      >
        ...
        {/* Outer Glow Halo */}
        <circle cx={cx} cy={cy} r={sphereR + 10} fill="none" stroke="oklch(0.72 0.16 195 / 20%)" strokeWidth="12" />
        ...
        {pole === "S" ? (
          ...
          <g>
            ...
            <circle cx={cx} cy={cy} r="4" fill="oklch(0.72 0.16 195)" stroke="white" strokeWidth="1.5" />
            <text
              x={cx + 7}
              y={cy + 3}
          ...
          <g>
            ...
            <ellipse cx={cx + 35} cy={cy + 110} rx="14" ry="10" fill="url(#antarcticIce)" stroke="white" />
        ...
        {visibleBases.map((base, idx) => {
          ...
          return (
            <g key={idx} className="cursor-pointer group">
              ...
              <circle cx={pt.x} cy={pt.y} r="14" fill="#fbbf24" opacity="0.2" className="animate-ping" />
        ...
        {!showIndianOnly &&
          visibleExpeditions.map((exp) => {
            ...
            const style = statusColors[exp.status] || statusColors.Active;
            ...
            return (
              ...
              >
                ...
                {(exp.status === "Active" || isSelected) && (
                  <circle cx={pt.x} cy={pt.y} r={isSelected ? "16" : "12"} fill={style.svg} opacity="0.28" className="animate-ping" />
                )}
      ...
      <div className="border-t border-border bg-background/80 px-5 py-3 backdrop-blur-md flex flex-wrap items-center justify-between text-xs text-muted-foreground">
        <div className="flex items-center gap-3 font-mono text-[11px]">
          <span className="text-foreground font-bold flex items-center gap-1">
            ...
            {pole === "S" ? "Antarctic Polar Projection (50°S–90°S)" : "Arctic Ocean Projection (50°N–90°N)"}
          </span>
...
export function ExpeditionPanel({
  ...
  return (
    ...
    >
      ...
      <p className="text-xs leading-relaxed text-muted-foreground">{expedition.summary}</p>
      ...
      <div className="space-y-2 pt-2 border-t border-border">
        ...
        <ul className="space-y-1 pl-1">
          {(expedition.objectives ?? (expedition.objective ? [expedition.objective] : [])).map((obj, i) => (
      ...
      <div className="pt-3 border-t border-border flex items-center justify-between font-mono text-[11px] text-muted-foreground">
        <span>Team: {expedition.members} Scientists</span>
        <span className="text-primary font-bold">{expedition.stationRef || "Field Expedition"}</span>
      </div>
    </motion.div>
  );
}

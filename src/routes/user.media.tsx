...
export const Route = createFileRoute("/user/media")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse polar images, videos and infographics from Arctic and Antarctic expeditions." },
...
function MediaPage() {
  ...
  const topics = useMemo(() => ["All", ...Array.from(new Set(mediaAssets.map((m) => m.topic)))], []);
  ...
  const results = useMemo(() => {
    ...
    return mediaAssets.filter((m) => {
      ...
      if (q && ![m.title, m.description, m.credit].join(" ").toLowerCase().includes(q)) return false;
      return true;
    });
      ...
      <div className="glass rounded-2xl p-4 glow-soft">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            ...
            <select value={topic} onChange={(e) => setTopic(e.target.value)} className={selectCls} aria-label="Filter by topic">
              {topics.map((t) => (

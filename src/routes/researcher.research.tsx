...
function MyResearchPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newItem: ResearchItem = {
      ...
      publisherName: getAuthSession()?.firstName
        ? [getAuthSession()?.firstName, getAuthSession()?.middleName, getAuthSession()?.lastName]
            .filter(Boolean)
            .join(" ")
        : undefined,
  ...
  return (
    <div className="space-y-8">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Submit a manuscript, report, or dataset for National Polar Repository
                      ingestion.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., Maitri Station, Antarctica"
                      className="w-full rounded-xl border border-input bg-background/80 px-3.5 py-2 text-xs text-foreground outline-none focus:border-primary/60"
                    />
                  </div>
                  <div>
      ...
      {detailItem && <ResearchDetailModal item={detailItem} onClose={() => setDetailItem(null)} />}
    </div>
  );
}

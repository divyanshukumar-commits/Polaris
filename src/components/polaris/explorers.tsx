...
import { Bookmark, Compass, Download, Filter, Flame, Globe2, Library, MountainSnow, RotateCcw, Satellite, Search, Waves } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between font-mono text-[11px] uppercase tracking-wider text-muted-foreground">
        <span>
          Showing <strong className="text-foreground">{results.length}</strong> verified research item
          {results.length === 1 ? "" : "s"}
        </span>
...
export function ExpeditionsExplorer() {
  const [selected, setSelected] = useState<Expedition | null>(expeditions[0]);
    ...
    <div className="space-y-6">
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4">
          {selected ? (
            <ExpeditionPanel expedition={selected} onClose={() => setSelected(null)} />
          ) : (
            <div className="glass rounded-3xl p-10 text-center text-muted-foreground border border-border">
              ...
              <p className="text-xs">Click on any colored expedition pin on the polar sphere to inspect details.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="pt-4">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className={selectCls} aria-label="Filter expeditions by status">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((e) => (
            <ExpeditionCard key={e.id} expedition={e} onView={selectExpedition} />
          ))}
        </div>
      </div>
    </div>
